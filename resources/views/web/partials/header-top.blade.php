<div class="w-full bg-gray-50">
    <div class="max-w-screen-xl px-4 py-2 mx-auto">
        <div class="flex items-center justify-center md:justify-between">
            <div class="text-sm text-gray-500 shrink-0">Welcome to Flowbite!</div>
            <ul class="items-center hidden text-sm text-gray-500 divide-x divide-solid md:flex">
                <li>
                    <a href="/faq"
                       class="px-4 transition duration-150 ease-in-out hover:underline hover:text-primary-500">FAQs</a>
                </li>
                <li>
                    <a href="/about"
                       class="px-4 transition duration-150 ease-in-out hover:underline hover:text-primary-500">About Us</a>
                </li>
                <li>
                    <a href="/contact"
                       class="px-4 transition duration-150 ease-in-out hover:underline hover:text-primary-500">Contact Us</a>
                </li>
            </ul>
        </div>
    </div>
</div>
