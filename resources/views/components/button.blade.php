...
@php
    ...
    if (strtolower($variant) == 'secondary') {
        $classes =
            $defaultClasses . ' px-4 py-2 bg-white border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50';
    }

    if (strtolower($variant) == 'soft') {
        $classes = $defaultClasses . ' px-4 py-2 bg-gray-100 rounded-md text-gray-700 hover:bg-gray-200';
    }

    if (strtolower($variant) == 'icon') {
        $classes = $defaultClasses . ' p-2 bg-white rounded-full text-gray-700 hover:bg-gray-100';
    }
@endphp
